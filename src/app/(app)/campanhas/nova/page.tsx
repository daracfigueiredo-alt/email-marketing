...
export default function NovaCampanhaPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!resp.ok) throw new Error("Falha ao criar campanha. Verifique se todas as etapas têm um modelo selecionado.");
      ...
      <Card className="max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button disabled={carregando} className="bg-brand-500 hover:bg-brand-600 text-white text-sm rounded-md px-4 py-2 disabled:opacity-60">
            {carregando ? "Salvando..." : "Criar campanha"}
          </button>
        </form>
      </Card>
    </div>
